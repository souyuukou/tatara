...
// CUDA grid の z 次元で扱える LayerStack bucket 数の上限。
constexpr unsigned int kMaxSupportedNumBuckets = 65535;
// L2/L3 の小 bucket 向け kernel が register 配列に保持する bucket 数。
// host はこの上限を超える場合、indexed または unsorted kernel を選択する。
constexpr unsigned int kTiledRegisterBucketMax = 9;
...
extern "C" __global__ void dense_mm_bwd_weight_bucket_indexed(
    const float* input,
    unsigned long long,
    const float* output_gradient,
    unsigned long long,
    ...
    const unsigned int bucket = blockIdx.z;
    if (bucket >= num_buckets || cell >= cells_per_bucket) {
        return;
    }
    const unsigned int output_index = cell / input_dimension;
    const unsigned int input_index = cell % input_dimension;
    const unsigned int bucket_start = bucket_offsets[bucket];
    const unsigned int bucket_end = bucket_offsets[bucket + 1U];
    const unsigned int bucket_length = bucket_end - bucket_start;
    const unsigned int positions_per_split =
        (bucket_length + gridDim.y - 1U) / gridDim.y;
    const unsigned int first_position = bucket_start + blockIdx.y * positions_per_split;
    if (first_position >= bucket_end) {
        return;
    }
    const unsigned int last_position =
        min(first_position + positions_per_split, bucket_end);
    float accumulator = 0.0F;
    for (unsigned int sorted_row = first_position; sorted_row < last_position; ++sorted_row) {
        const int original_row = permutation[sorted_row];
        if (original_row >= 0) {
            accumulator +=
                input[static_cast<unsigned long long>(original_row) * input_dimension + input_index] *
                output_gradient[
                    static_cast<unsigned long long>(original_row) * output_dimension + output_index];
        }
    }
    atomicAdd(
        weight_gradient + static_cast<unsigned long long>(bucket) * cells_per_bucket + cell,
        accumulator
    );
}

extern "C" __global__ void dense_mm_bwd_weight_bucket_unsorted(
    const float* input,
    unsigned long long,
    const float* output_gradient,
    unsigned long long,
    ...
    for (unsigned int row = first_row; row < last_row; ++row) {
        if (bucket_idx[row] == static_cast<int>(bucket)) {
            accumulator +=
                input[static_cast<unsigned long long>(row) * input_dimension + input_index] *
                output_gradient[
                    static_cast<unsigned long long>(row) * output_dimension + output_index];
...
extern "C" __global__ void dense_mm_bwd_weight_bucket_tiled_l2(
    ...
    const unsigned int first_row = blockIdx.y * rows_per_split;
    const unsigned int last_row = min(first_row + rows_per_split, batch);
    float accumulators[kTiledRegisterBucketMax] = {};
    const unsigned int bucket_limit = min(num_buckets, kTiledRegisterBucketMax);
    for (unsigned int row = first_row; row < last_row; ++row) {
        const int bucket = bucket_idx[row];
