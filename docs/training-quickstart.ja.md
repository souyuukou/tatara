...
| `--num-buckets` (`layerstack`) | 9 | `progress8kpabs` では `[2, 65535]` の整数で、各局面を `min(N-1, floor(progress * N))` へ routing する。256 超は direct (非 sorted) GPU 経路となり per-bucket 重みで VRAM が大きく増える。`N > 256` の `.bin` は対応版 rshogi が必要。`kingrank9` では 9 固定 |
