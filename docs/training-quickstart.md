...
| `--num-buckets` (`layerstack`) | 9 | In `progress8kpabs` mode, an integer in `[2, 65535]`; positions route to `min(N-1, floor(progress * N))`. Values above 256 use a direct (non-sorted) GPU path and need substantially more VRAM for per-bucket weights; their nets require a matching rshogi build for inference. In `kingrank9` mode this must be 9 |
