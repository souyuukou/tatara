...
pub(crate) struct LayerstackArgs {
    ...
    /// routed to `min(N-1, floor(p * N))` and N must be in
    /// `[2, {MAX_SUPPORTED_NUM_BUCKETS}]`; values above 256 use a direct
    /// (non-sorted) GPU path and need substantially more VRAM. In kingrank9
    /// mode this value must be 9. The default 9 keeps the
