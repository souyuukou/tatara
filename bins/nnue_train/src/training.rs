...
#[cfg(feature = "gpu")]
use crate::arch::{BUCKET_SORT_MAX_N, L1_SKIP};
#[cfg(any(feature = "gpu", test))]
use crate::cli::*;
#[cfg(feature = "gpu")]
...
fn gpu_oom_error(
    ...
    if let Some(num_buckets) = feature_config.num_buckets {
        let _ = write!(msg, ", num-buckets={num_buckets}");
    }
    if feature_config.psqt_enabled {
    ...
    if matches!(feature_config.num_buckets, Some(n) if n > 9) {
        let _ = writeln!(
            msg,
...
mod tests {
    ...
    fn gpu_oom_error_lists_relevant_remedies() {
        ...
        assert!(m.contains("num-buckets=9"));
    ...
    #[test]
    fn layerstack_persistent_vram_budget_accepts_max_num_buckets() {
        ...
        assert!(gib < 96.0);
        assert!(
            check_layerstack_persistent_vram_budget(73_305, 1536, 16, 32, 65_535, false).is_ok()
        );
    }
}
