...
/// を持ち得る production launch が別 crate / 別 binary に移ると、この走査から外れて 81 本の
...
/// `native_kernels.cu` の対応 bucket 上限と小 bucket 向け register 配列上限が host 側の
/// 分岐定数と一致することを固定する。
#[test]
fn native_bucket_capacities_match_host() {
    let native = include_str!("../../../../crates/cuda-native-runtime/kernels/native_kernels.cu");
    let parse_constant = |name: &str| {
        let marker = format!("constexpr unsigned int {name} = ");
        native
            .lines()
            .find_map(|line| line.trim_start().strip_prefix(&marker))
            .and_then(|rest| rest.split(';').next())
            .map(str::trim)
            .unwrap_or_else(|| panic!("native_kernels.cu must define {name}"))
            .parse::<usize>()
            .unwrap_or_else(|_| panic!("{name} must be an integer"))
    };
    assert_eq!(
        parse_constant("kMaxSupportedNumBuckets"),
        crate::arch::MAX_SUPPORTED_NUM_BUCKETS,
        "native kMaxSupportedNumBuckets must match host MAX_SUPPORTED_NUM_BUCKETS"
    );
    assert_eq!(
        parse_constant("kTiledRegisterBucketMax"),
        crate::arch::TILED_REGISTER_BUCKET_MAX,
        "native kTiledRegisterBucketMax must match host TILED_REGISTER_BUCKET_MAX"
    );
}
...
#[test]
fn extended_bucket_layouts_run_one_native_training_step() -> Result<(), Box<dyn std::error::Error>>
{
    let context = CudaContext::new(0)?;
    for num_buckets in [
        crate::arch::TILED_REGISTER_BUCKET_MAX + 1,
        crate::arch::BUCKET_SORT_MAX_N + 1,
    ] {
        ...
        assert!(
            loss.is_finite(),
            "native LayerStack loss is not finite for {num_buckets} buckets: {loss}"
        );
        trainer.assert_all_weights_finite()?;
    }
    Ok(())
